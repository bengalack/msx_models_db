...
A single static web page presenting MSX1, MSX2, MSX2+, and MSX turbo R computer model specifications. The purpose is to easily get the overview of the myriad of models ever released. Compare, review and understand the similarities. Find outliers and hallmarks. We see that we have opened a pandoras box - the number of ways to improve this thing we've started, is wast. Hosted at [bengalack.github.io/msx_models_db](https://bengalack.github.io/msx_models_db/)

__Features:__
* Sortable, filterable, selectable spreadsheet-like grid. 
* All states are encoded in the URL so any view can be bookmarked or shared.
* The URL is "future-proof": Additions of data, including new columns, are expected, and will still work in the future.
* Works on `file://`, GitHub Pages, as a Blogger/Blogspot embed, as well as normal webservers.

The second and maybe the most important part of this project is the *scraper*, written in python, which builds the `data.js` dataset used by the web page.

__Main data sources are:__
...
This project exposes data legally from openmsx (GPL2). It exposes data from msx.org legally as it is a tool from the same msx community. For the community, by the community.
