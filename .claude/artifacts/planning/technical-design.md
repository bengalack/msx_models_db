...
The scraped msx.org "Chipset" prose is parsed into two sortable columns: **Engine (semi-custom ASIC)** (id 105, gate arrays) and **Engine (full-custom ASIC)** (id 99, MSX-Engines). The raw text stays in the hidden column `engine_raw` (id 106) and is shipped per model as the cell tooltip of both columns.

Full rules, decisions and the expected output for all 46 live source values: `.claude/artifacts/planning/2026-09-21-engine-columns-design.md`.
...
| `scraper/build.py` | Migrates cached `engine` → `engine_raw`; serialises `tooltips` |
| `src/types.ts` | `ModelRecord.tooltips?` |
| `src/grid.ts` | Sets `td.dataset.tooltip` from `tooltips`; cell tooltip wins over overflow/truncation |
...
A column may list values that sort after all real values in both directions (blanks still sort last of all): `sort_last=("None",)` on the Engine columns keeps "None" out of the alphabetical run of chip ids. Serialised as `ColumnDef.sortLast`; `sortModels()` ranks those values before comparing.
