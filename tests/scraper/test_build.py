...
class TestBuildDefaultOff:
    ...
    def test_default_off_matches_column_config(self, tmp_path):
        ...
        content = output_path.read_text(encoding="utf-8")
        data = json.loads(content[content.index("{"):content.rindex(";")])
        configured = {c.key for c in active_columns() if c.default_off}
        serialised = {col["key"] for col in data["columns"] if col.get("defaultOff")}
        assert serialised == configured
        for col in data["columns"]:
            if col["key"] not in configured:
